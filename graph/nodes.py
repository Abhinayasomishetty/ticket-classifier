...
async def knowledge_node(state: TicketState) -> Dict[str, Any]:
    ...
    try:
        ...
        print("=" * 60)
        duration = int((time.time() - start_time) * 1000)
        logs = log_node_execution(
            state, "Knowledge_RAG", "error", None, duration, str(e)
        )
        return {
            "knowledge_answer": "Knowledge search failed",
            "knowledge_sources": [],
            **logs,
        }
...
ANSWER:"""
    ...
    try:
        ...
        duration = int((time.time() - start_time) * 1000)
        logs = log_node_execution(
            state, "Log_Analyzer", "error", None, duration, str(e)
        )
        return {"log_insights": f"Error:{repr(e)}", **logs**logs}
